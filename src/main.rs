...
impl Game {
    fn render(&mut self, args: &RenderArgs) {
        ...
        const SKYBLUE: [f32; 4] = [0.0, 1.0, 1.0, 1.0];

        self.gl.draw(args.viewport(), |_c, gl| {
            graphics::clear(SKYBLUE, gl);
        });
...
impl Snake {
    fn render(&mut self, args: &RenderArgs) {
        ...
        let squares: Vec<graphics::types::Rectangle> = self.snake_parts
            ...
            .map(|p| graphics::rectangle::square(
                    p.0 as f64,
                    p.1 as f64,
                    self.width as f64))
            .collect();
    ...
    pub fn update(&mut self, just_eaten: bool, cols: u32, rows: u32) -> bool {
        let mut new_front: SnakePiece = 
            (*self.snake_parts.front().expect("No front of snake fround.")).clone();
